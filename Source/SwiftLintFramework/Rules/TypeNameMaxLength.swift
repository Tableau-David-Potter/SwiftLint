...
public struct TypeNameMaxLengthRule: ASTRule {
    public let identifier = "type_name_max_length"

    public struct Constants {
        public static let defaultMaximum = 40
    }
    ...
    public init() {
        self.init(
            parameters: [RuleParameter(severity: .Warning, value: Constants.defaultMaximum)])
    }
                        ...
                        if name.characters.count > parameter.value {
                            violations.append(
                                StyleViolation(name: identifier,
                                    ...
                                    location: location,
                                    severity: .Warning,
                                    reason: "Type name should be no longer than \(parameter.value) characters in length: " +
    ...
    public let example = RuleExample(
        ruleName: "Type Name Max Length Rule",
        ruleDescription: "Type name should be no longer than \(Constants.defaultMaximum) characters in length.",
        nonTriggeringExamples: [
            "struct MyStruct {}"
        ],
        triggeringExamples: [
            "struct myVeryVeryVeryLongIdentifierForATypeName {}"
        ],
        showExamples: false
    )
}
