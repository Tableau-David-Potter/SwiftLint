...
public struct VariableNameMaxLengthRule: ASTRule, ParameterizedRule {
    public let identifier = "variable_name_max_length"
                    ...
                    for parameter in parameters {
                        if name.characters.count > parameter.value {
                            violations.append(StyleViolation(name: identifier,
                                ...
                                reason: "Variable name should be no longer than \(parameter.value) characters in length: " +
    ...
    public let example = RuleExample(
        ruleName: "Variable Name Max Length Rule",
        ruleDescription: "Variable name should be no longer than \(Constants.defaultMaximum) characters in length.",
        nonTriggeringExamples: [
            "let myVariable = 0"
        ],
        triggeringExamples: [
            "let myVeryVeryVeryLongIdentifierForAVariableName = 0"
        ],
        showExamples: false
    )
}
